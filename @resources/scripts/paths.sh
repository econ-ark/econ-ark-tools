...
# =============================================================================
# PROJECT PARENT DETECTION VIA .project_container FILE
# =============================================================================
...
find_project_container() {
    ...
    while [[ "$current_dir" != "/" ]]; do
        if [[ -f "$current_dir/.project_container" ]]; then
            # Validate that .project_container is in the correct location
            local dir_name="$(basename "$current_dir")"
            if [[ "$dir_name" =~ -(make|Latest|Public)$ ]]; then
                # .project_container file is inside a project repo directory - this is wrong!
                ...
                # Copy command to clipboard (macOS and Linux compatible)
...
get_project_name_from_file() {
    local project_container_dir="$1"
    local project_file="$project_container_dir/.project_container"
    
    if [[ -f "$project_file" ]]; then
        # Read project name from file, trim whitespace
        local project_name="$(cat "$project_file" | tr -d '\n\r' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')"
        if [[ -n "$project_name" ]]; then
            echo "$project_name"
            return 0
        fi
        
        # If file exists but is empty, derive project name from directory structure
        # Look for a directory matching *-make, *-Latest, or *-Public pattern
        for dir in "$project_container_dir"/*-make "$project_container_dir"/*-Latest "$project_container_dir"/*-Public; do
            if [[ -d "$dir" ]]; then
                # Extract project name from directory name (remove suffix)
    ...
    echo "ERROR: Could not read project name from $project_file" >&2
    return 1
}
...
detect_execution_context() {
    ...
    # Get project name from .project_container file
    local project_name
    project_name="$(get_project_name_from_file "$project_container_dir")" || return 1
    
    local dir_name="$(basename "$current_dir")"
    
    # Determine repo type based on directory name pattern
    # Expected format: [ProjectName]-{make,Latest,Public}
    local repo_type=""
    if [[ "$dir_name" =~ ^${project_name}-(make|Latest|Public)$ ]]; then
        repo_type="${BASH_REMATCH[1]}"
    else
        echo "ERROR: Directory '$dir_name' doesn't match expected pattern ${project_name}-{make|Latest|Public}" >&2
        return 1
    fi
    
    # Check if sibling directories exist (project mode)
    if [[ -d "$project_container_dir/${project_name}-make" ]] && \
       ...
       [[ -d "$project_container_dir/${project_name}-Public" ]]; then
        ...
        echo "project:$project_container_dir:$repo_type:$project_name:$current_dir"
        return 0
    else
        # STANDALONE MODE: Only this repo exists
        echo "standalone:$project_container_dir:$repo_type:$project_name:$current_dir"
        return 0
    fi
}
...
EXPORTED VARIABLES:
    PROJECT_NAME        Name of the project (from .project_container file)
    ...
    MAKE_ROOT           HAFiscal-make directory (build system root)
    LATEST_ROOT         HAFiscal-Latest directory
    PUBLIC_ROOT         HAFiscal-Public directory
    QE_ROOT             HAFiscal-QE directory
...
REQUIREMENTS:
    - .project_container file in parent directory containing project name
    - Git repository (for path detection via git rev-parse --show-toplevel)
...
IFS=':' read -r context_type PROJECT_CONTAINER repo_type PROJECT_NAME REPO_ROOT <<< "$context_info"
