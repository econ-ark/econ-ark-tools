#!/bin/bash
# Secure boot ISO was able to boot for MacBookPro 15, 2018 with options
#    noapic efi=noruntime nomodeset
# But install to USB drive failed at "grub-install" with message
# 'Failed to register the EFI boot entry: Operation not permitted"
# Extensive google searches did not uncover a solution; not clear
# whether disabling system protection would or would not solve it
...
cp $pathToScript/root/EFI/BOOT/Econ-ARK.VolumeIcon.icns   $iso_make/iso_new/EFI/BOOT/.VolumeIcon.icns
cp $pathToScript/root/EFI/BOOT/Econ-ARK.VolumeIcon.icns   $iso_make/iso_new/.VolumeIcon.icns
