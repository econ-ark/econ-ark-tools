...
xfconf-query --channel xfce4-session --property /shutdown/LockScreen &> /dev/null # Querying its value gives an errmsg if it does not exist
...
xfce4powermanagerexistsCode="$?" # If exit code is zero, it is running -- so quit it (because it causes crash errmsg if reboot happens while running)
