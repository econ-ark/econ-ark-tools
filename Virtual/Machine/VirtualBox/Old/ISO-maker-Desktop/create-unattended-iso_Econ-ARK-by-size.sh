...
# pathToScript=/home/econ-ark/GitHub/econ-ark/econ-ark-tools/Virtual/Machine/VirtualBox/ISO-maker-Desktop/
...
while true; do
    ...
    echo
    echo "  [1] Ubuntu $prec LTS Desktop amd64 - Precise Pangolin"
    echo "  [2] Ubuntu $trus LTS Desktop amd64 - Trusty Tahr"
    echo "  [3] Ubuntu $xenn LTS Desktop amd64 - Xenial Xerus"
    echo "  [4] Ubuntu $bion LTS Desktop amd64 - Bionic Beaver"
    echo
...
cat $SystemLogRateLimitInterval 0 >  /etc/rsyslog.conf
cat $SystemLogRateLimitBurst    0 >> /etc/rsyslog.conf
...
sed -i "/label install/ilabel autoinstall\n\
  menu label ^Autoinstall Econ-ARK Xubuntu Desktop\n\
  kernel /install/vmlinuz\n\
  append file=/cdrom/preseed/ubuntu-desktop.seed initrd=/install/initrd.gz DEBCONF_DEBUG=5 auto=true priority=high preseed/file=/cdrom/preseed/econ-ark.seed preseed/file/checksum=$seed_checksum -- ks=cdrom:/ks.cfg " $iso_make/iso_new/isolinux/txt.cfg
  
# add the autoinstall option to the menu for USB Boot
#sed -i '/set timeout=30/amenuentry "Autoinstall Econ-ARK Xubuntu Desktop" {\n\	set gfxpayload=keep\n\	linux /install/vmlinuz append file=/cdrom/preseed/ubuntu-desktop.seed initrd=/install/initrd.gz auto=true priority=high preseed/file=/cdrom/preseed/econ-ark.seed quiet ---\n\	initrd	/install/initrd.gz\n\}' $iso_make/iso_new/boot/grub/grub.cfg
sed -i  '/set timeout=30/amenuentry "Autoinstall Econ-ARK Xubuntu Desktop" {\n\	set gfxpayload=keep\n\	linux /install/vmlinuz append file=/cdrom/preseed/ubuntu-desktop.seed initrd=/install/initrd.gz auto=true priority=high preseed/file=/cdrom/preseed/econ-ark.seed       ---\n\	initrd	/install/initrd.gz\n\}' $iso_make/iso_new/boot/grub/grub.cfg
...
cmd+=" econ-ark-google-drive:econ-ark@jhuecon.org/Resources/Virtual/Machine/$datestr-$size/"
